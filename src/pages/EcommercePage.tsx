...
export default function EcommercePage(){
  ...
  // Expense tracking state - force ecommerce business type
  ...
  async function addExpenseWithAI() {
    ...
    try {
      ...
      if (!category) {
        const aiResult = await categorizeExpense(description, amount, 'ecommerce')
        category = aiResult.category || ''
        
        if (aiResult.category && aiResult.confidence > 0.6) {
          const confirmed = confirm(`AI suggests category: "${getCategoryLabel(aiResult.category, 'ecommerce')}" (${Math.round(aiResult.confidence * 100)}% confidence)\n\nReason: ${aiResult.reasoning}\n\nUse this category?`)
          if (!confirmed) {
            category = ''
          }
        }
  ...
  async function addNaturalLanguageExpense() {
    ...
    try {
      const parsed = await parseNaturalLanguageExpense(naturalLanguageInput, 'ecommerce')
  ...
  useEffect(() => {
    suggestExpenses('ecommerce').then(setSuggestedExpenses)
  }, [])
  ...
  function downloadCSVFile(){
    const rows = [
      ...
      ['Gross business income', i.netBusinessIncome],
    ...
    try {
      ...
      a.href = url
      a.download = 'taxtrail-ecommerce-plan.csv'
      a.click()
    } finally {
  ...
  function downloadPDFFile(){
    const doc = new jsPDF()
    doc.setFontSize(18); doc.text('TaxTrail - E-commerce Tax Plan 2025', 14, 16)
    doc.setFontSize(12)
    doc.text(`Filing status: ${i.filingStatus.toUpperCase()}`, 14, 26)
    doc.text(`Gross business income: ${asUSD(i.netBusinessIncome)}`, 14, 34)
    ...
    if (expenseData.entries.length > 0) {
      doc.addPage()
      doc.setFontSize(18); doc.text('E-commerce Business Expenses', 14, 16)
      doc.setFontSize(10)
      let yPos = 30
      expenseData.entries.forEach(expense => {
        if (yPos > 280) {
    ...
    doc.save('taxtrail-ecommerce-plan.pdf')
  }
    ...
    <>
      ...
      <div className="grid grid-2" style={{alignItems:'center'}}>
        <div>
          ...
          <p style={{margin:'8px 0', fontSize:'14px', opacity:0.8}}>
            Specialized tax planning for Etsy, Amazon, eBay, and online store owners with advanced inventory tracking and COGS management.
          </p>
        </div>
        {ctaLinks.length > 0 && (
      ...
      <div className="grid grid-2" style={{marginTop:16}}>
        ...
        <div className="card">
          ...
          <div className="grid grid-2">
            ...
            <div>
              <label htmlFor={FIELD_IDS.netBusinessIncome}>Gross e-commerce revenue (before expenses)</label>
      ...
      <div className="card" style={{marginTop:16}}>
        <div className="row" style={{alignItems:'center', justifyContent:'space-between', flexWrap:'wrap'}}>
          <h2>E-commerce Business Expenses 🛒</h2>
          ...
          <>
            ...
            <div style={{marginTop:12}}>
              ...
              <div style={{marginBottom:16}}>
                ...
                <div className="row" style={{gap:8}}>
                  <input 
                    ...
                    placeholder="e.g., 'Amazon fees $150' or 'inventory purchase $2000'"
                    style={{flex:1}}
                  />
                  <button 
                    type="button" 
              ...
              {suggestedExpenses.length > 0 && (
                <div style={{marginBottom:16}}>
                  <label>💡 Common e-commerce expenses:</label>
            ...
            <div className="grid grid-3" style={{marginTop:12}}>
              ...
              <div>
                ...
                >
                  ...
                  {Object.entries(EXPENSE_CATEGORIES.ecommerce || {}).map(([groupKey, group]) => (
      ...
      {showUpgradeModal && (
        <div className="modal-backdrop" role="presentation" onClick={closeUpgradeModal}>
          <div className="modal-card" role="dialog" aria-modal="true" aria-labelledby="premium-modal-title" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-upsell">Advanced inventory tracking and COGS management for e-commerce businesses.</div>
