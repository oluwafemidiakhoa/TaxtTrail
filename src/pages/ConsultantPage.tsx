...
export default function ConsultantPage(){
  ...
  // Expense tracking state - force consultant business type
  ...
  async function addExpenseWithAI() {
    ...
    try {
      ...
      if (!category) {
        const aiResult = await categorizeExpense(description, amount, 'consultant')
        category = aiResult.category || ''
        
        if (aiResult.category && aiResult.confidence > 0.6) {
          const confirmed = confirm(`AI suggests category: "${getCategoryLabel(aiResult.category, 'consultant')}" (${Math.round(aiResult.confidence * 100)}% confidence)\n\nReason: ${aiResult.reasoning}\n\nUse this category?`)
          if (!confirmed) {
            category = ''
          }
        }
  ...
  async function addNaturalLanguageExpense() {
    ...
    try {
      const parsed = await parseNaturalLanguageExpense(naturalLanguageInput, 'consultant')
  ...
  useEffect(() => {
    suggestExpenses('consultant').then(setSuggestedExpenses)
  }, [])
  ...
  function downloadCSVFile(){
    const rows = [
      ...
      ['Gross consulting income', i.netBusinessIncome],
      ['Total business expenses', totals.totalExpenses],
    ...
    if (expenseData.entries.length > 0) {
      rows.push(['', ''])
      rows.push(['CONSULTING BUSINESS EXPENSES', ''])
    ...
    try {
      ...
      a.href = url
      a.download = 'taxtrail-consultant-plan.csv'
      a.click()
    } finally {
  ...
  function downloadPDFFile(){
    const doc = new jsPDF()
    doc.setFontSize(18); doc.text('TaxTrail - Solo Consultant Tax Plan 2025', 14, 16)
    doc.setFontSize(12)
    doc.text(`Filing status: ${i.filingStatus.toUpperCase()}`, 14, 26)
    doc.text(`Gross consulting income: ${asUSD(i.netBusinessIncome)}`, 14, 34)
    doc.text(`Business expenses: ${asUSD(totals.totalExpenses)}`, 14, 42)
    ...
    if (expenseData.entries.length > 0) {
      doc.addPage()
      doc.setFontSize(18); doc.text('Consulting Business Expenses', 14, 16)
      doc.setFontSize(10)
      let yPos = 30
      expenseData.entries.forEach(expense => {
        if (yPos > 280) {
    ...
    doc.save('taxtrail-consultant-plan.pdf')
  }
  ...
  return (
    <>
      ...
      <div className="grid grid-2" style={{alignItems:'center'}}>
        <div>
          <h1>TaxTrail - Solo Consultant</h1>
          ...
          <p style={{margin:'8px 0', fontSize:'14px', opacity:0.8}}>
            Professional tax planning for freelancers, consultants, and service providers with comprehensive expense tracking and quarterly tax consultations.
          </p>
        </div>
        {ctaLinks.length > 0 && (
      ...
      <div className="grid grid-2" style={{marginTop:16}}>
        ...
        <div className="card">
          ...
          <div className="grid grid-2">
            ...
            <div>
              <label htmlFor={FIELD_IDS.netBusinessIncome}>Gross consulting income (before expenses)</label>
      ...
      <div className="card" style={{marginTop:16}}>
        <div className="row" style={{alignItems:'center', justifyContent:'space-between', flexWrap:'wrap'}}>
          <h2>Consulting Business Expenses 💼</h2>
          ...
          <>
            ...
            <div style={{marginTop:12}}>
              ...
              <div style={{marginBottom:16}}>
                ...
                <div className="row" style={{gap:8}}>
                  <input 
                    ...
                    placeholder="e.g., 'client dinner $120' or 'conference registration $500'"
                    style={{flex:1}}
                  />
                  <button 
                    type="button" 
              ...
              {suggestedExpenses.length > 0 && (
                <div style={{marginBottom:16}}>
                  <label>💡 Common consulting expenses:</label>
            ...
            <div className="grid grid-3" style={{marginTop:12}}>
              ...
              <div>
                ...
                >
                  ...
                  {Object.entries(EXPENSE_CATEGORIES.consultant || {}).map(([groupKey, group]) => (
      ...
      {showUpgradeModal && (
        <div className="modal-backdrop" role="presentation" onClick={closeUpgradeModal}>
          <div className="modal-card" role="dialog" aria-modal="true" aria-labelledby="premium-modal-title" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-upsell">Professional expense tracking and quarterly tax consultations for solo consultants.</div>
